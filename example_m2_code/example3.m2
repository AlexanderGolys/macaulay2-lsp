...
h Ring := R -> (
    ...
    if R.?name then
        f(toString, R);
    if (try (coefficientRing R;
        true
    ) else false
    ) then
        f(coefficientRing, R);
    if instance(R, QuotientRing) then
        (m(ambient, ideal)) R;
)
...
h Matrix := t -> (
    ...
    if (target t).?generators then
        (m((degrees, source, generators, target), (generators, target))) t;
    if (target t).?relations then
        (m((degrees, source, relations, target), (relations, target))) t;
    if (source t).?generators then
        (m((degrees, source, generators, source), (generators, source))) t;
    if (source t).?relations then
        (m((degrees, source, relations, source), (relations, source))) t;
)
h Module := M -> (
    (m(class, ring, (degrees, ambient))) M;
    if M.?generators then
        (m((degrees, source, generators), generators)) M;
    if M.?relations then
        (m((degrees, source, relations), relations)) M;
)
h ChainComplex := C -> (
    complete C;
    (m(class, ring)) C;
