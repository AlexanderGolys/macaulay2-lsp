...
testModulesForGenus14Curves = (N, p) -> (
    ...
    utime := timing while (i < N) do (
        ...
        i = i + 1;
        if test == true then (j = j + 1;
        );
    );
    timeForNModules := utime#0;
    numberOfGoodModules := j;
...
getDeltaForDeg17CY = (b, f1) -> (
    ...
    k := numgens target fb.dd_3 - 28;
    --# of linear syzygies
    b1 := fb.dd_1;
    b2 := fb.dd_2_{0..27};
    b2' := fb.dd_2_{28..28 + k - 1};
    ...
    while i < 3 do (
        ...
        while j < 16 do(
            ...
            while k < 7 do (
                ...
                if l == 0 then (equations = u_l*Delta;
                ) else (
...
checkMorphismsForDeg17CY = (b, skewSymMorphisms) -> (
    ...
    isNonDegenerate := #select(
        (flatten degrees source gens equations), i -> i == 1
    ) == 0;
    collectGarbage();
    ...
    if isOK then (
        ...
        isGoodMorphism := false;
        while isGoodMorphism == false do (
