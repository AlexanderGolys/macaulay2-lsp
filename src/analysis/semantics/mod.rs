//! Semantic facts and document-facing analysis queries.
...
use super::first_pass::{
    control_flow_scope, nested_symbols, walk, BindingEffect, BindingFact, ScopeTree,
};
...
use crate::node_metadata::{M2Node, NodeKind, SyntaxNodeId};
...
struct SemanticRegistry {
    ...
    binding_states_by_node: HashMap<SyntaxNodeId, BindingStateId>,
...
impl OutputReference {
    ...
    pub fn referenced_value<'tree>(self, node: M2Node<'tree>) -> Option<M2Node<'tree>> {
        let cell = node.enclosing(NodeKind::Cell)?;
        let root = cell.root();
...
impl Analysis {
    pub fn find_definition(&self, name: &str, pos: Position) -> Option<TextRange> {
        ...
        self.binding(binding_id)?
            ...
            .iter()
            .enumerate()
            .filter(|(_, state)| state.effective_from() <= pos)
    ...
    pub fn new_with_knowledge(
        ...
        analysis.registry.scopes = walk(root, source, |fact, scopes| {
            analysis.collect_binding_fact(fact, scopes, source);
        });
        analysis.enrich_types(root, source, 0, 0, knowledge);
        analysis.collect_semantics(root, source, knowledge);
        analysis.registry.pending_source_semantic_roles.clear();
        analysis.collect_diagnostics(root, source, knowledge);
        analysis
    }

    fn collect_binding_fact(
        &mut self,
        fact: BindingFact,
        scopes: &ScopeTree,
        source: &(impl SourceNavigation + ?Sized),
    ) {
        self.collect_definition(fact, source, scopes);
    }
    ...
    fn collect_semantics(
        ...
        for node in root.descendants() {
            ...
            self.record_assignment_fact(
                ...
                self.find_scope_at(source.position_for_node(node))
                    .unwrap_or(0),
    ...
    fn enrich_parameters(
        ...
        let parameter_nodes = nested_symbols(node, NodeKind::is_parameter_container);
    ...
    fn collect_definition(
        ...
        let name = fact.target.text();
        match fact.effect {
            BindingEffect::Declare => {
                self.add_binding(name, fact, source);
            }
            BindingEffect::Assign => {
                let position = source.position_for_node(fact.target);
                let binding_id = self
                ...
                if let Some(binding_id) = binding_id {
                    self.add_binding_state(binding_id, fact, source);
                } else {
                    self.add_binding(name, fact, source);
                }
            }
    ...
    fn enrich_definitions(
        ...
        for definition in nested_symbols(node, NodeKind::is_collection_expression) {
    ...
    fn enrich_symbol(
        &mut self,
        node: M2Node,
        _source: &(impl SourceNavigation + ?Sized),
        enrichment: BindingEnrichment,
    ) {
        let Some(binding_state) = self
            .registry
            .binding_states_by_node
            .get(&node.id())
            .copied()
        else {
            return;
    ...
    fn add_binding(
        ...
        let range = source.range_for_node(fact.target);
        let state = BindingStateInfo {
            ...
            indexed_element_type: None,
            source_type: None,
            value_range: fact.value.map(|value| source.range_for_node(value)),
            definition_range: enclosing_definition_range(fact.target, source),
            span: range,
            scope_idx: fact.scope,
            node_id: fact.target.id(),
        };
        ...
        self.registry.binding_states_by_node.insert(
            fact.target.id(),
            BindingStateId {
                binding: binding_id,
                state: 0,
    ...
    fn add_binding_state(
        ...
        let state = BindingStateInfo {
            ...
            value_range: fact.value.map(|value| source.range_for_node(value)),
            definition_range: enclosing_definition_range(fact.target, source),
            span: source.range_for_node(fact.target),
            scope_idx: fact.scope,
            node_id: fact.target.id(),
        };
    ...
    fn reindex_binding_states(&mut self, binding_id: BindingId) {
        let nodes = self
            .binding(binding_id)
            .into_iter()
            .flat_map(|binding| binding.states.iter().map(|state| state.node_id))
            .collect::<Vec<_>>();
        for (state, node_id) in nodes.into_iter().enumerate() {
            self.registry.binding_states_by_node.insert(
                node_id,
                BindingStateId {
                    binding: binding_id,
...
fn fixed_parameter_names(lambda: M2Node<'_>) -> Option<Vec<ObjectName>> {
    ...
    let parameters = nested_symbols(
        lambda.child_by_field_name("parameters")?,
...
fn enclosing_definition_range(
    ...
    source.range_for_node(node.enclosing(NodeKind::Cell).unwrap_or(node))
}
