...
- **Build = fmt + clippy + test.** Every task's "verify" step runs `cargo fmt`, `cargo clippy --all-targets`, and `cargo test -p m2-ls`. **Baseline caveat:** `main` already carries 9 clippy warnings in WIP files (typesystem.rs, hover.rs, …) and does NOT pass `-D warnings`; those are out of scope and must not be "fixed" (typesystem.rs is off-limits to tidy). The achievable gate is therefore: fmt clean, `cargo test -p m2-ls` green, and **your diff introduces no new clippy warnings** — confirm by checking that no clippy warning references a file you modified. Where a task step below literally shows `clippy --all-targets -- -D warnings`, read it against this constraint (pre-existing warnings in untouched files do not block you).
